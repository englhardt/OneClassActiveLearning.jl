...
function get_query_objects(qs::BatchPQs,
    ...
    @debug "[QS] Selecting best batch of $(qs.k) from $(length(candidate_indices)) candidates."
...
function set_rep_measure!(strategy::MultiObjectiveBatchQs, name::Symbol)::Function
    if (name == :KDE)
        strategy.rep_measure = (data::Array{T, 2} where T <: Real, labels::Dict{Symbol, Vector{Int}}, candidate_indices::Vector{Int64}) -> begin
...
function set_iterative_div_measure!(strategy::MultiObjectiveBatchQs, name::Symbol)::Function
    if (name == :AngleDiversity)
        strategy.div_measure = (model::SVDD.OCClassifier, candidate_indices::Vector{Int64}, j::Int64, old_scores::Vector{Float64}) -> begin
