OPT = OneClassActiveLearning.QueryStrategies.QuerySynthesisStrategies.Optimization
    ...
    @testset "particle swarm optimization" begin
        for d in [1, 2, 10]
            @testset "$d dim" begin
                ...
                x_opt, _ = OPT.pso(f, -ones(d), ones(d))
                @test size(x_opt) == (d,)
        ...
        for opt in [EvolutionaryOptimization(:cmaes, μ=1, λ=2, iterations=MAX_ITER),
            ...
            for d in [1, 2, 10]
                @testset "$(typeof(opt)): $d dim" begin
                    Random.seed!(0)
                    f(x) = -vec(sum(abs.(x), dims=1))
                    x_opt = OPT.query_synthesis_optimize(f, opt, randn(d, 10), fill(:U, 10))
                    @test size(x_opt) == (d, 1)
                end
            end
        end
